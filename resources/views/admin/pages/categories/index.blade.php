...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">{{__('dashboard.sidebar.app')}} /</span> {{__('dashboard.category.categories')}}</h4>

        <div class="card">
            <div class="table-header d-flex align-items-center justify-content-between">
                <h5 class="card-header">{{__('dashboard.crud.add')}} {{__('dashboard.category.category')}}</h5>
{{--                <a href="{{route('admin.category.create')}}" class="btn btn btn-primary me-5">{{__('dashboard.crud.add')}} {{__('dashboard.category.category')}}</a>--}}
            </div>
                ...
                @if (!empty($categories) && count($categories) > 0)
                        <table class="table">
                            ...
                            @foreach ($categories as  $category)
                                <tr class="text-center" data-product-id="{{ $category->id }}">
                                    <td>
                                    ...
                                    @if(app()->getLocale()  == 'ar')
                                        <td class="">{{ $category->name_ar }}</td>

                                    @else
                                        <td class="">{{$category->name_en}}</td>
                                    @endif
                                    <td>
                                        @if($category->image)
                                            <img src="{{url('/dash-img/category/'.$category->image)}}"  class="rounded" height="20" width="20"/>
                                        @else
                                                    ...
                                                    <div class="row">
                                                        <div class="col mb-3">
                                                            <label for="nameWithTitle" class="form-label">{{__('dashboard.table.delete')}}</label>
                                                                <input type="text" disabled id="nameWithTitle" value="{{$category->name}}" class="form-control">
                                                        </div>
                                                    </div>
                                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Sortable(cateList, {
                ...
                onEnd: function(evt) {
                    ...
                    console.log(cateIds);
                    fetch('{{ route('update_category_order') }}', {
                        method: 'POST',
                        headers: {
